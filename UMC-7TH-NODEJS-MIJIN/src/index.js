...
// 사용자 회원가입
router.post("/api/v1/users/signup", handleUserSignUp);

// 사용자 정보 조회 API 추가
router.get("/api/v1/users/:userId", getUser);

// 지역 추가 API
router.post('/api/v1/regions', handleCreateRegion);

// 리뷰 추가
router.post("/api/v1/reviews", handleCreateReview);

// 가게 추가 API
router.post("/api/v1/stores", handleCreateStore);

// 가게에 미션 추가 API
router.post("/api/v1/missions", handleCreateMission);

// 미션 도전 API
router.post("/api/v1/missions/challenge", handleChallengeMission);

// 내가 작성한 리뷰 목록 가져오기
router.get("/api/v1/reviews/my-reviews", handleGetReviewsByUser);

// 특정 가게의 리뷰 목록 조회
router.get("/api/v1/stores/:storeId/reviews", handleGetReviewsByStore);

// 내가 진행 중인 미션 목록 조회
router.get('/api/v1/missions/in-progress/:userId', getInProgressMissions);

// 내가 진행 중인 미션을 진행 완료로 변경
router.patch('/api/v1/missions/complete/:userId/:missionId', markMissionAsCompleted);
