...
export const addUserMission = async (userMissionData) => {
  const { userId, missionId } = userMissionData;
  const conn = await pool.getConnection();

  try {
    const [result] = await conn.query(
      `INSERT INTO userMission (userId, missionId, isCompleted, createdAt, updatedAt) VALUES (?, ?, ?, NOW(), NOW());`,
      [userId, missionId, false] // is_completed는 false로 초기화
    );

    return { userMissionId: result.insertId, ...userMissionData };
  } catch (err) {
